...
test.describe("horde game mode", () => {
	...
	test("visual: deterministic arena frame", async ({ page }) => {
		// Darwin baseline is committed; the linux baseline is generated by the
		// update-snapshots workflow (UPDATE_SNAPSHOTS=1 bypasses this gate) and
		// committed alongside it — until then linux CI skips rather than fails.
		test.skip(
			process.platform !== "darwin" && !process.env.UPDATE_SNAPSHOTS,
			"linux baseline generated via update-snapshots workflow",
		);
		await page.goto("/game?seed=42&testMode=1");
		await page.waitForFunction(() => window.__game !== undefined);
